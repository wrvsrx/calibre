...
stdenv.mkDerivation (finalAttrs: {
  ...
  buildPhase = ''
    # user-agent-data.json is get from https://code.calibre-ebook.com/ua-popularity
    ...
    ${python3Packages.python.pythonOnBuildForHost.interpreter} setup.py bootstrap \
      ...
      --path-to-user-agent-data=${./user-agent-data.json} \
      --path-to-translations=${translations} \
      --path-to-cacerts=${cacert}/etc/ssl/certs/ca-bundle.crt \
      --path-to-isocodes=${iso-codes-zip}
